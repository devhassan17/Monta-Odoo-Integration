...
class MontaQtySync:
    def __init__(self, env, company=None):
        ...
        if not cfg:
            ...
            self.timeout = 20
            _logger.warning(
                "MontaQtySync: Config missing or company not allowed: %s",
    ...
    def _get_product_stock(self, sku: str) -> Optional[MontaStock]:
        ...
        # Alternate structure: Stock -> StockAvailable
    ...
    def _set_template_threshold(self, template, minimum: float):
        # Keep same behavior: best-effort write
        try:
    ...
    def _is_kit(self, product) -> bool:
        ...
        if not bom:
            bom = MrpBom.search(
                [("product_tmpl_id", "=", product.product_tmpl_id.id), ("product_id", "=", False)],
                limit=1,
            )
