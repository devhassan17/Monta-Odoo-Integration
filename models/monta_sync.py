    ...
    def _monta_sync_batch(self):
        ...
        for so in self:
            ...
            try:
                ...
                is_delivered = (
                    normalized_status in DELIVERED_NORMALIZED
                    or status_code.upper() in DELIVERED_CODES
                    or meta.get("is_delivered") in (True, "true", "True", "1", 1)
                )
                ...
                pickings = Picking.search(["|", ("sale_id", "=", so.id), ("origin", "=", so.name)], order="id asc")
                if not pickings:
                ...
                for pick in pickings:
                    try:
                        ...
                        # fill qty_done on move lines if needed
                            ...
                            for mv in pick.move_lines:
                                ...
                                if float_utils.float_compare(getattr(mv, "quantity_done", 0.0), wanted, precision_rounding=rounding) < 0:
                                    try:
                                        ...
                                        _logger.warning(
                                            "[Monta] Unable to set quantity_done on move %s (picking %s)",
                                            mv.id,
                                            pick.name,
                                        )
                        ...
                        try:
                            ...
                            try:
                                ...
                                _logger.exception(
                                    "[Monta] Failed to validate picking %s for sale %s: %s / %s",
                                    pick.name,
                                    so.name,
                                    e_val,
                                    e2,
                        ...
                        _logger.exception(
                            "[Monta] Exception while processing picking %s for sale %s: %s",
                            pick.name if pick else "n/a",
                            so.name,
                            e_pick,
                        )
                ...
                _logger.exception(
                    "[Monta] Exception while attempting to auto-validate pickings for %s: %s",
                    so.name,
                    e,
                )
