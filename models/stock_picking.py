...
class StockPicking(models.Model):
    ...
    def _compute_monta_fields_from_status(self):
        for picking in self:
            ...
            domain = [("order_name", "=", ref)]
            account_key = Snapshot._current_account_key() if hasattr(Snapshot, "_current_account_key") else ""
            if account_key and hasattr(Snapshot, "_has_monta_account_key_column") and Snapshot._has_monta_account_key_column():
            ...
            if snapshot:
                ...
                picking.monta_track_trace = snapshot.track_trace
                picking.monta_delivery_date = snapshot.delivery_date
            else:
                picking.monta_status = "Sent to Monta"
                picking.monta_status_code = False
                picking.monta_track_trace = False
    ...
    def _write_to_snapshot(self, field_name, value):
        for picking in self:
            ref = picking.monta_webshop_order_id
            if not ref:
                continue
            ...
            if rec:
                ...
                is_sub = bool(picking.sale_id and picking.sale_id._fields.get("is_subscription") and picking.sale_id.is_subscription)
                Snapshot.create({
                    "order_name": ref,
                    field_name: value,
                    "order_kind": "renewal" if is_sub else "sale",
    ...
    def _inverse_monta_status(self):
        for picking in self:
            picking._write_to_snapshot("status", picking.monta_status)

    def _inverse_monta_status_code(self):
        for picking in self:
            try:
                code = int(picking.monta_status_code) if picking.monta_status_code else 0
            except Exception:
                code = 0
            picking._write_to_snapshot("status_code", code)

    def _inverse_monta_track_trace(self):
        for picking in self:
            picking._write_to_snapshot("track_trace", picking.monta_track_trace)

    def _inverse_monta_delivery_date(self):
        for picking in self:
            picking._write_to_snapshot("delivery_date", picking.monta_delivery_date)
